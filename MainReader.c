 ...
 * -------------------------------------------------------------
 */
amber_void bErrorPrint(amber_string fmt, ...);
amber_void displayBuffer(Buffer* ptr_Buffer);
amber_long getFileSize(amber_string fname);
amber_intg isNumber(const amber_string ns);
amber_void startReader(amber_string, amber_string, amber_char, amber_intg, amber_intg);
...
amber_intg mainReader(amber_intg argc, amber_string* argv) {
	...
	amber_string program = argv[0];
	amber_string input = argv[2];
	amber_char mode = MODE_FIXED;
	amber_intg size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
amber_void startReader(amber_string program, amber_string input, amber_char mode, amber_intg size, amber_intg increment) {

	BufferPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	amber_intg loadSize = 0;		/* the size of the file loaded in the buffer */
	amber_char symbol;			/* symbol read from input file */
...
amber_void bErrorPrint(amber_string fmt, ...) {
	...
	(amber_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
